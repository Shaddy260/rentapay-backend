...
async function listRows(req, res) {
  try {
    ...
    logger.error('[adminSql] listRows error:', err.message);
    captureException(err);
    return res.status(500).json({ error: 'Failed to fetch rows. Please try again.' });
  }
}
...
async function updateRow(req, res) {
  try {
    ...
    logger.error('[adminSql] updateRow error:', err.message);
    captureException(err);
    return res.status(500).json({ error: 'Failed to update row. Please try again.' });
  }
}
