...
async function registerLandlord(req, res) {
  ...
  try {
    let { fullName, phone, email, password, gender, unitsCount, periodMonths, whatsappNumber } = req.body;
...
async function login(req, res) {
  try {
    ...
    // email into a false "invalid credentials" - phone doesn't have
    // this problem because normalizePhone() strips non-digits below.
...
const GOOGLE_NO_ACCOUNT_MSG =
  'No RentaPay account is registered with this Google email. Sign up as a landlord, or ask your landlord to add you with this email address.';
...
async function loginWithGoogle(req, res) {
  try {
    ...
    if (!process.env.GOOGLE_CLIENT_ID) {
      console.error('[auth] loginWithGoogle: GOOGLE_CLIENT_ID is not set in the environment.');
      return res.status(500).json({ error: 'Google login is not configured on this server yet.' });
    }
    if (accountType && !ALL_ACCOUNT_TYPES.includes(accountType)) {
      return res.status(400).json({ error: "accountType, if provided, must be 'landlord', 'manager', or 'tenant'." });
    }
    ...
    try {
      ...
      console.error('[auth] loginWithGoogle: token verification failed:', err.message);
    ...
    if (accountType) {
      ...
      ({ table, phoneField } = accountTable(accountType));
      const { data: acc } = await supabase.from(table).select('*').ilike('email', email).maybeSingle();
      if (!acc) {
    ...
    supabase.from(table).update({ failed_login_attempts: 0, locked_until: null }).eq('id', account.id)
      .then(({ error: resetErr }) => { if (resetErr) console.error('[auth] loginWithGoogle: failed to reset attempt counter (non-fatal):', resetErr.message); });
    ...
    console.error('[auth] loginWithGoogle error:', err.message);
...
async function getMyLandlordProfile(req, res) {
  try {
    ...
    const { data: landlord, error } = await supabase
      .from('landlords')
      .select('id, full_name, phone, email, photo_url, payment_method, paybill_number, paybill_account_number, till_number, gender, notification_style, onboarding_dismissed_at, whatsapp_number')
      .eq('id', landlordId)
      .single();
...
async function updateMyContact(req, res) {
  try {
    ...
    const { data: updated, error } = await supabase.from('landlords').update(updateFields).eq('id', landlordId).select('id, full_name, phone, email, gender, notification_style, whatsapp_number').single();
    if (error) {
