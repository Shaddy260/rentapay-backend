...
async function getMyLandlordProfile(req, res) {
  try {
    ...
    const { data: landlord, error } = await supabase
      .from('landlords')
      .select('id, full_name, phone, email, photo_url, payment_method, paybill_number, paybill_account_number, till_number, gender, notification_style, onboarding_dismissed_at, whatsapp_number, kra_pin')
      .eq('id', landlordId)
      .single();
    if (error || !landlord) return res.status(404).json({ error: 'Account not found.' });
