...
async function insertChatMessage({ threadType, landlordId, tenantId, role, roleLevel, senderId, senderName, body, replyToId = null }) {
  ...
  (async () => {
    try {
      ...
      logger.error('[chat] insertChatMessage: push failed (non-blocking):', pushErr.message);
      captureException(pushErr);
    }
  })();
...
async function listMessages(req, res) {
  try {
    ...
    logger.error('[chat] listMessages error:', err.message);
    captureException(err);
...
// ---------------------------------------------------------------------
// Item 12 (direct request: "Chat with an Agent should always be pinned
// at the top of the conversation list, regardless of other message
// activity"): every branch below builds its thread list with the
// agent conversation (admin_landlord for landlord/manager,
// admin_tenant for tenant) pushed in first, but then re-sorts
// everything by most-recent-message - which quietly un-pins it the
// moment any other thread gets newer activity. This sorts by recency
// same as before, then pulls the agent thread (if present) back to
// index 0 afterward, so it's a real pin rather than something that
// only happened to look pinned before the first reply came in.
function sortThreadsAgentPinned(threads, isAgentThread) {
  const sorted = [...threads].sort((a, b) => new Date(b.lastMessageAt || 0) - new Date(a.lastMessageAt || 0));
  const agentIndex = sorted.findIndex(isAgentThread);
  if (agentIndex > 0) {
    const [agent] = sorted.splice(agentIndex, 1);
    sorted.unshift(agent);
  }
  return sorted;
}

// PERFORMANCE FIX (platform audit): listThreads' landlord/manager
// branches used to run 2 sequential DB queries (last message + unread
// count) PER THREAD - for a landlord with N active tenants, that's
// 2N+2 serial round trips on every single load of the Messages page,
// which visibly slowed the page down as a landlord's tenant list
// grew. This mirrors the in-memory-aggregation approach the
// admin_landlord/admin_tenant thread list just above already uses
// (see its own "small dataset... fine to do client-side aggregation"
// comment) - one query fetches every relevant message for this
// landlord across every thread type at once, then a single pass in
// memory finds each thread's latest message and unread count, since
// messages are already ordered newest-first.
async function attachChatThreadSummaries(threads, { landlordId, readFlagColumn }) {
  if (threads.length === 0) return;
  const threadTypes = [...new Set(threads.map((t) => t.threadType))];
  const { data: messages } = await supabase
    .from('chat_messages')
    .select(`thread_type, tenant_id, body, created_at, ${readFlagColumn}`)
    .eq('landlord_id', landlordId)
    .in('thread_type', threadTypes)
    .order('created_at', { ascending: false });

  const rows = messages || [];
  const keyOf = (threadType, tenantId) => `${threadType}:${tenantId || ''}`;
  const lastByKey = new Map();
  const unreadByKey = new Map();

  for (const row of rows) {
    const key = keyOf(row.thread_type, row.tenant_id);
    if (!lastByKey.has(key)) lastByKey.set(key, row); // rows are newest-first, so the first hit per key is the latest
    if (row[readFlagColumn] === false) unreadByKey.set(key, (unreadByKey.get(key) || 0) + 1);
  }

  for (const t of threads) {
    const key = keyOf(t.threadType, t.tenantId);
    const last = lastByKey.get(key);
    t.lastMessage = last?.body || null;
    t.lastMessageAt = last?.created_at || null;
    t.unreadCount = unreadByKey.get(key) || 0;
  }
}
...
async function listThreads(req, res) {
  try {
    ...
    if (role === 'landlord') {
      ...
      // Attach last message + unread count per thread - single batched
      // query instead of one pair of queries per thread (see
      // attachChatThreadSummaries's own comment for why that mattered).
      await attachChatThreadSummaries(threads, { landlordId: id, readFlagColumn: 'read_by_landlord' });

      const sortedThreads = sortThreadsAgentPinned(threads, (t) => t.threadType === 'admin_landlord');
      return res.json({ threads: sortedThreads });
    }
    ...
    if (role === 'manager') {
      ...
      // behalf here.
      await attachChatThreadSummaries(threads, { landlordId, readFlagColumn: 'read_by_landlord' });
    ...
    if (role === 'tenant') {
      ...
      const sortedThreads = sortThreadsAgentPinned(threads, (t) => t.threadType === 'admin_tenant');
      return res.json({ threads: sortedThreads });
    }
