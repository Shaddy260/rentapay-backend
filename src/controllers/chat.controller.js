...
// ---------------------------------------------------------------------
async function insertChatMessage({ threadType, landlordId, tenantId, scoutId, role, roleLevel, senderId, senderName, body, replyToId = null }) {
  ...
  (async () => {
    try {
      if (threadType === 'landlord_tenant') {
        ...
        await sendPushToRecipient('landlord', landlordId, { title: `New message from a Scout`, body });
      }
    } catch (pushErr) {
