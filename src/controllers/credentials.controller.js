...
async function listFirstTimeCredentials(req, res) {
  try {
    ...
    // DIRECT REQUEST FIX: manager/caretaker rows no longer carry an
    // expires_at at all (see propertyManager.controller.js's
    // addManager - they're auto-verified now, so there's no OTP
    // window to expire). A plain `.gt('expires_at', now)` would
    // silently exclude every one of those rows forever, since SQL
    // NULL > anything is never true - not "no expiry", just gone from
    // this list entirely. Match either a still-valid expiry OR no
    // expiry at all.
    let query = supabase
    ...
    logger.error('[credentials] listFirstTimeCredentials error:', err.message);
...
async function listAllFirstTimeCredentialsForAdmin(req, res) {
  try {
    ...
    logger.error('[credentials] listAllFirstTimeCredentialsForAdmin error:', err.message);
    captureException(err);
...
async function listPasswordResetRequests(req, res) {
  try {
    ...
    logger.error('[credentials] listPasswordResetRequests error:', err.message);
    captureException(err);
...
async function listAllPasswordResetRequestsForAdmin(req, res) {
  try {
    ...
    const grouped = { landlord: [], tenant: [], manager: [], caretaker: [] };
    for (const row of enriched) {
      if (grouped[row.role]) grouped[row.role].push(row);
    }
    ...
    logger.error('[credentials] listAllPasswordResetRequestsForAdmin error:', err.message);
    captureException(err);
