...
async function getLandlordDashboard(req, res) {
  try {
    ...
    // PERFORMANCE FIX (pre-deploy review): this used to call
    // units.find(...) inside the tenants loop - O(tenants x units) on
    // every single dashboard load. Fine for a handful of units, but it
    // scales quadratically, so a landlord with a large portfolio (or a
    // manager viewing an all-properties view) would feel the
    // dashboard get progressively slower as they grow. A Map built
    // once up front makes each lookup O(1) instead.
    ...
    logger.error('[dashboard] getLandlordDashboard error:', err.message);
...
async function getPaymentsThisMonth(req, res) {
  try {
    ...
    logger.error('[dashboard] getPaymentsThisMonth error:', err.message);
    captureException(err);
...
async function globalSearch(req, res) {
  try {
    ...
    logger.error('[dashboard] globalSearch error:', err.message);
    captureException(err);
...
async function getDueDatesCalendar(req, res) {
  try {
    ...
    logger.error('[dashboard] getDueDatesCalendar error:', err.message);
    captureException(err);
...
async function getAttentionFeed(req, res) {
  try {
    ...
    const [tenantsRes, pendingRes, landlordRes, helpRes, maintenanceRes] = await Promise.all([
      scopedToNoProperties
        ? Promise.resolve({ data: [] })
        : (() => {
            let q = supabase
              .from('tenants')
              .select('id, full_name, balance_due, rent_override, primary_phone, units!inner(id, unit_name, property_id, due_day_of_month, rent_amount, extra_charges)')
              .eq('landlord_id', landlordId)
              .eq('is_active', true)
              .gt('balance_due', 0);
    ...
    const overdueTenants = (tenantsRes.data || [])
      ...
      .map((t) => {
        ...
        return { tenantId: t.id, tenantName: t.full_name, unitId: t.units.id, unitName: t.units.unit_name, amountDue: Number(t.balance_due), daysOverdue: daysOverdue(dueDate, today), primaryPhone: t.primary_phone || null };
      })
      .filter((t) => t.daysOverdue > 0)
      .sort((a, b) => b.daysOverdue - a.daysOverdue);

    // FEATURE (Section 8: Tenant risk flag) - a tenant whose balance
    // has grown to cover 2+ full rent cycles is treated as having
    // "missed 2+ consecutive rent payments". There's no per-cycle
    // ledger of individual missed charges, so this is the best
    // available proxy from data already tracked: the monthly billing
    // job (monthlyBilling.job.js) is the ONLY place balance_due ever
    // increases (by exactly one month's rent + extras each cycle), so
    // a balance of 2x+ that monthly charge means at least 2 cycles'
    // worth of rent has never been cleared by a payment.
    const riskFlaggedTenants = (tenantsRes.data || [])
      .map((t) => {
        const unit = t.units;
        if (!unit) return null;
        const rentAmount = Number(t.rent_override || unit.rent_amount || 0);
        const extrasTotal = (unit.extra_charges || []).reduce((sum, c) => sum + Number(c.amount || 0), 0);
        const monthlyCharge = rentAmount + extrasTotal;
        if (monthlyCharge <= 0) return null;
        const cyclesOwed = Number(t.balance_due) / monthlyCharge;
        if (cyclesOwed < 2) return null;
        return {
          tenantId: t.id,
          tenantName: t.full_name,
          unitId: unit.id,
          ...
          amountDue: Number(t.balance_due),
          missedCycles: Math.floor(cyclesOwed),
          primaryPhone: t.primary_phone || null,
        };
      })
      .filter(Boolean)
      .sort((a, b) => b.missedCycles - a.missedCycles);
    ...
    logger.error('[dashboard] getAttentionFeed error:', err.message);
    captureException(err);
...
// Below these sample sizes, a rate is statistically thin enough that a
// bare percentage reads as alarming/broken rather than informative
// (spec: "Misleading rate calculations on small sample sizes"). Kept as
// named constants so the frontend threshold-driven copy always matches
// what the backend actually used to decide.
const PAYMENT_SAMPLE_THRESHOLD = 10; // fewer than this many tracked payments
const UNIT_SAMPLE_THRESHOLD = 10; // fewer than this many units
...
async function computeLandlordStatistics(req) {
  ...
  const occupancyRateLowSample = totalUnits > 0 && totalUnits < UNIT_SAMPLE_THRESHOLD;
  const occupancyRateCounts = `${occupied + noticeGiven} of ${totalUnits}`;

  // Earliest unit added in the current scope - used as a proxy for
  // "when this property/portfolio slice started being tracked", so the
  // 6-month collection chart can be trimmed/annotated instead of
  // showing pre-activation months as unexplained zeros.
  const earliestUnitAddedAt = units.reduce((earliest, u) => {
    if (!u.created_at) return earliest;
    return !earliest || new Date(u.created_at) < new Date(earliest) ? u.created_at : earliest;
  }, null);
  ...
  let onTimeCount = 0;
  let lateCount = 0;
  const lateByUnit = new Map(); // unit_id -> { count, lastLateAt }
  for (const p of payments) {
    ...
    if (dueDay) {
      if (paidDate.getDate() <= dueDay) {
        onTimeCount += 1;
      } else {
        lateCount += 1;
        const entry = lateByUnit.get(p.unit_id) || { count: 0, lastLateAt: null };
        entry.count += 1;
        if (!entry.lastLateAt || paidDate > new Date(entry.lastLateAt)) entry.lastLateAt = p.paid_at;
        lateByUnit.set(p.unit_id, entry);
      }
    }
  }

  // "Chronically late" units - 2+ late payments inside the 6-month
  // window - surfaced as their own ranked list rather than buried in
  // the aggregate on-time/late donut, so a landlord can see exactly
  // which units to follow up on instead of just a percentage.
  const unitById = new Map(units.map((u) => [u.id, u]));
  const chronicallyLateUnits = Array.from(lateByUnit.entries())
    .filter(([, v]) => v.count >= 2)
    .map(([unitId, v]) => ({
      unitId,
      unitName: unitById.get(unitId)?.unit_name || 'Unknown unit',
      lateCount: v.count,
      lastLateAt: v.lastLateAt,
    }))
  ...
  const onTimeRateLowSample = totalTrackedPayments > 0 && totalTrackedPayments < PAYMENT_SAMPLE_THRESHOLD;
  const onTimeRateCounts = `${onTimeCount} of ${totalTrackedPayments}`;

  // Trim/annotate the 6-month chart if the property/portfolio slice
  // was only added partway through the window, so consecutive $0
  // months read as "not tracked yet" rather than a data failure.
  let monthlyChartNote = null;
  let monthlyTrimmed = monthly;
  if (earliestUnitAddedAt) {
    const addedDate = new Date(earliestUnitAddedAt);
    const addedKey = `${addedDate.getFullYear()}-${addedDate.getMonth()}`;
    const addedIndex = monthly.findIndex((m) => m.key === addedKey);
    if (addedIndex > 0) {
      monthlyTrimmed = monthly.slice(addedIndex);
      monthlyChartNote = `Property added ${addedDate.toLocaleDateString('en-GB', { month: 'long', year: 'numeric' })}`;
    }
  }
  ...
  // FEATURE (Section 8: rent collection trend chart) - "rent owed" per
  // month, alongside monthlyCollected above. There's no historical
  // per-month billed-amount ledger (units/rents change over time and
  // nothing snapshots what was billed in a past month), so this uses
  // the current portfolio's expected monthly rent (same figure as
  // expectedThisMonth) as a level baseline applied across the window -
  // the best available approximation from data this app actually
  // tracks, and consistent with how expectedThisMonth is already used
  // for the current month's collection rate above.
  const monthlyOwed = monthly.map((m) => ({ key: m.key, label: m.label, value: expectedThisMonth }));
  const monthlyOwedTrimmed = monthlyTrimmed.length === monthly.length
    ? monthlyOwed
    : monthlyOwed.slice(monthly.length - monthlyTrimmed.length);
  const collectionRate = expectedThisMonth > 0 ? Math.round((collectedThisMonth / expectedThisMonth) * 1000) / 10 : null;
  // Proxy sample size for collection rate: how many units are actually
  // expected to be paying rent this month. Few contributing units means
  // one payment (or one missed payment) swings the percentage wildly.
  const rentPayingUnits = occupied + noticeGiven;
  const collectionRateLowSample = expectedThisMonth > 0 && rentPayingUnits < UNIT_SAMPLE_THRESHOLD;
  const collectionRateCounts = `KES ${Number(collectedThisMonth).toLocaleString()} of KES ${Number(expectedThisMonth).toLocaleString()}`;
  ...
  // Trim in lockstep with monthlyCollected (same starting index) so
  // pdfReport.service.js, which zips the two arrays together by
  // position, doesn't pair the wrong months up after a trim.
  const monthlyExpensesTrimmed = monthlyTrimmed.length === monthly.length
    ? monthlyExpenses
    : monthlyExpenses.slice(monthly.length - monthlyTrimmed.length);
  ...
  return {
    data: {
      units: {
        ...
        noticeGiven,
        occupancyRate,
        occupancyRateCounts, // e.g. "7 of 22"
        occupancyRateLowSample,
      },
      payments: {
        onTimeCount,
        lateCount,
        onTimeRate, // percentage, or null if not enough data yet
        onTimeRateCounts, // e.g. "1 of 5"
        onTimeRateLowSample,
        overdueNow: overdueCount,
        collectedThisMonth,
        expectedThisMonth,
        collectionRate,
        collectionRateCounts, // e.g. "KES 4,500 of KES 10,000"
        collectionRateLowSample,
      },
      expenses: {
        expensesThisMonth,
        netProfitThisMonth,
      },
      monthlyCollected: monthlyTrimmed,
      monthlyOwed: monthlyOwedTrimmed, // approximate - see comment above
      monthlyChartNote, // e.g. "Property added July 2026", or null
      monthlyExpenses: monthlyExpensesTrimmed,
      chronicallyLateUnits,
      sampleThresholds: { payments: PAYMENT_SAMPLE_THRESHOLD, units: UNIT_SAMPLE_THRESHOLD },
    },
  };
}
...
async function getLandlordStatistics(req, res) {
  try {
    ...
    logger.error('[dashboard] getLandlordStatistics error:', err.message);
    captureException(err);
...
async function getLandlordStatisticsPdf(req, res) {
  try {
    ...
    logger.error('[dashboard] getLandlordStatisticsPdf error:', err.message);
    captureException(err);
