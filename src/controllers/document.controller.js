...
// TENANT: upload their own document (e.g. ID copy, signed lease scan)
// - tenantId is forced to the caller's own id, never trusted from the
// body, same defensive stance used everywhere else a tenant could
// otherwise name someone else's record.
async function uploadDocument(req, res) {
  try {
    const isTenant = req.user.role === 'tenant';
    const tenantId = isTenant ? req.user.id : req.body.tenantId;
    ...
    const landlordId = isTenant ? tenant.landlord_id : effectiveLandlordId(req);
    ...
    logger.error('[document] uploadDocument error:', err.message);
...
async function deleteDocument(req, res) {
  try {
    ...
    if (accessError) return res.status(accessError.statusCode).json(accessError);
    if (await blockIfSubscriptionExpired(req, res, landlordId, existing.property_id)) return;

    const { error: storageError } = await supabase.storage.from(BUCKET_NAME).remove([existing.file_path]);
    if (storageError) { logger.error('[document] failed to remove storage object:', storageError.message); captureException(storageError); }
