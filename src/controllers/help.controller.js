  ...
  try {
    ...
    if (SUPPORT_EMAIL) {
      try {
        ...
        logger.error('[help] CRITICAL: notification email failed - request was saved but admin was NOT notified by email:', emailErr.message);
    ...
    logger.error('[help] submitHelpRequest error:', err.message);
...
async function listHelpRequests(req, res) {
  try {
    ...
    logger.error('[help] listHelpRequests error:', err.message);
    captureException(err);
...
async function resolveHelpRequest(req, res) {
  try {
    ...
    logger.error('[help] resolveHelpRequest error:', err.message);
    captureException(err);
...
async function listMyHelpRequests(req, res) {
  try {
    ...
    logger.error('[help] listMyHelpRequests error:', err.message);
    captureException(err);
...
async function deleteHelpRequest(req, res) {
  try {
    ...
    logger.error('[help] deleteHelpRequest error:', err.message);
    captureException(err);
...
async function getReplyThread(req, res) {
  try {
    ...
    logger.error('[help] getReplyThread error:', err.message);
    captureException(err);
