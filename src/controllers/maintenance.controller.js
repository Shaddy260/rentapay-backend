...
async function submitMaintenanceRequest(req, res) {
  try {
    ...
    logger.error('[maintenance] submitMaintenanceRequest error:', err.message);
...
async function listMyMaintenanceRequests(req, res) {
  try {
    ...
    logger.error('[maintenance] listMyMaintenanceRequests error:', err.message);
    captureException(err);
...
async function listMaintenanceRequests(req, res) {
  try {
    ...
    logger.error('[maintenance] listMaintenanceRequests error:', err.message);
    captureException(err);
    return res.status(500).json({ error: 'Failed to fetch maintenance requests.' });
  }
}
...
async function updateMaintenanceStatus(req, res) {
  try {
    ...
    logger.error('[maintenance] updateMaintenanceStatus error:', err.message);
    captureException(err);
