...
async function processRentPaymentCallback(payment, resultCode, callbackMetadata) {
  ...
  // Section 6: snapshot the rent period this payment covers and the
  // tenant's balance immediately after it, so the receipt always
  // prints what was true at payment time - not whatever the tenant's
  // balance happens to be by the time someone downloads the PDF.
  const paidAtDate = new Date();
  const rentPeriodLabel = paidAtDate.toLocaleDateString('en-GB', { month: 'long', year: 'numeric' });
...
async function recordManualPayment(req, res) {
  try {
    ...
    // Section 6: same rent-period label as the STK path, based on the
    // date the landlord recorded as when the payment happened (not
    // "now") - a landlord backfilling last month's cash payment should
    // get a receipt that says last month, not today.
    const paymentDateObjForPeriod = new Date(paymentDate);
    const rentPeriodLabel = paymentDateObjForPeriod.toLocaleDateString('en-GB', { month: 'long', year: 'numeric' });
    ...
    // Section 6: snapshot balance-after on the payment row itself too.
    await supabase.from('payments').update({ balance_after: newBalance }).eq('id', payment.id);
