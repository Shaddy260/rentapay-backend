...
async function createRequest(req, res) {
  try {
    ...
    logger.error('[paymentPlan] createRequest error:', err.message);
...
async function listRequests(req, res) {
  try {
    ...
    logger.error('[paymentPlan] listRequests error:', err.message);
    captureException(err);
...
async function decideRequest(req, res) {
  try {
    ...
    if (await blockIfSubscriptionExpired(req, res, landlordId, request.units?.property_id || null)) return;
    ...
    logger.error('[paymentPlan] decideRequest error:', err.message);
    captureException(err);
...
async function cancelRequest(req, res) {
  try {
    ...
    logger.error('[paymentPlan] cancelRequest error:', err.message);
    captureException(err);
...
// ---------------------------------------------------------------------
// DELETE /api/payment-plans/:requestId/installments/:index
//
// FEATURE (spec item 4): the tenant portal's Payment Plan section
// used to be view-only once a plan was approved - long-pressing an
// installment now offers a delete option for that one entry (e.g. an
// installment the tenant already cleared some other way, or a
// duplicate). Tenant-only, and only on their own request while it's
// still 'approved' - a pending/declined/cancelled request has no
// installments actively in force to prune. Removing every remaining
// installment leaves the plan with an empty list rather than deleting
// the request itself - the request row (and its chat history) stays
...
async function deleteInstallment(req, res) {
  try {
    ...
    if (role !== 'tenant') return res.status(403).json({ error: 'Only the tenant who made this request can edit it.' });
    ...
    const { data: request, error: fetchError } = await supabase
      .from('payment_plan_requests')
      .select('id, tenant_id, status, installments')
      .eq('id', requestId)
      .maybeSingle();
    if (fetchError) throw fetchError;
    if (!request) return res.status(404).json({ error: 'Payment plan request not found.' });
    if (request.tenant_id !== tenantId) return res.status(403).json({ error: 'This is not your payment plan.' });
    if (request.status !== 'approved') return res.status(400).json({ error: 'Only an approved payment plan\'s installments can be edited.' });

    const installments = Array.isArray(request.installments) ? request.installments : [];
    if (idx >= installments.length) return res.status(404).json({ error: 'That installment no longer exists.' });

    const updatedInstallments = installments.filter((_, i) => i !== idx);
    ...
    logActivity({ actorType: role, actorId: tenantId, action: 'payment_plan_installment_deleted', targetType: 'payment_plan_request', targetId: requestId });

    return res.json({ message: 'Installment removed.', request: updated });
  } catch (err) {
    logger.error('[paymentPlan] deleteInstallment error:', err.message);
    captureException(err);
    return res.status(500).json({ error: 'Failed to remove installment.' });
  }
}

module.exports = { createRequest, listRequests, decideRequest, cancelRequest, deleteInstallment };
