...
// Shared by createProperty/updateProperty: a loose sanity check, not a
// strict validator - just enough to catch someone pasting something
// that obviously isn't a link (e.g. a phone number or a plain address)
// while still accepting maps.app.goo.gl short links, full
// google.com/maps URLs, or any other http(s) mapping service the
// landlord happens to use.
function looksLikeUrl(value) {
  if (!value) return true; // empty/undefined is fine - the field is optional
  try {
...
async function listProperties(req, res) {
  try {
    ...
    logger.error('[property] listProperties error:', err.message);
...
async function createProperty(req, res) {
  try {
    ...
    if (mapsLink && !looksLikeUrl(mapsLink)) {
      return res.status(400).json({ error: 'mapsLink must be a valid link (e.g. copied from Google Maps\' Share option).' });
    }
    ...
    logger.error('[property] createProperty error:', err.message);
    captureException(err);
...
async function updateProperty(req, res) {
  try {
    ...
    const { name, location, county, constituency, description, mapsLink, rulesText, primaryContactManagerId } = req.body;
    let { caretakerName, caretakerPhone } = req.body;
    ...
    if (mapsLink !== undefined && mapsLink && !looksLikeUrl(mapsLink)) {
      return res.status(400).json({ error: 'mapsLink must be a valid link (e.g. copied from Google Maps\' Share option).' });
    }
    ...
    if (mapsLink !== undefined) updateFields.maps_link = mapsLink || null;
    // Optional free-text rules/regulations for this property. Empty
    // string is treated the same as not set, so the tenant-facing
    // rules card simply doesn't render rather than showing a blank box.
    if (rulesText !== undefined) updateFields.rules_text = rulesText ? rulesText.trim() : null;
    ...
    logger.error('[property] updateProperty error:', err.message);
    captureException(err);
...
// ---------------------------------------------------------------------
// GET /api/properties/:propertyId/reputation
// FOLLOW-UP to the tenant-rates-property feature: landlords/managers
// had no dashboard view of a property's own reputation at all (only
// tenants could see it, via getMyPropertyReputation, and the public
// listing page). Same aggregate-only shape as landlordReputation -
// never a rating tied back to an identifiable tenant.
// ---------------------------------------------------------------------
async function getPropertyReputationForLandlord(req, res) {
  try {
    const landlordId = effectiveLandlordId(req);
    const { propertyId } = req.params;
    ...
    if (property.landlord_id !== landlordId) return res.status(403).json({ error: 'You do not manage this property.' });

    const reputation = await propertyReputationService.getPropertyReputation(propertyId);
    return res.json({ property: { id: property.id, name: property.name }, reputation });
  } catch (err) {
    logger.error('[property] getPropertyReputationForLandlord error:', err.message);
    captureException(err);
...
async function assignUnitToProperty(req, res) {
  try {
    ...
    if (propertyId) {
      ...
      if (await blockIfSubscriptionExpired(req, res, landlordId, propertyId)) return;
    }
    ...
    logger.error('[property] assignUnitToProperty error:', err.message);
    captureException(err);
...
async function completePropertyPurchase(propPayment) {
  ...
  const { data: property, error: propError } = await supabase
    ...
    .insert({
      ...
      maps_link: propPayment.maps_link,
...
async function checkPropertyPaymentStatus(req, res) {
  try {
    ...
    try {
      ...
      logger.warn('[property] checkPropertyPaymentStatus: querySTKPushStatus failed, still pending:', queryErr.message);
    ...
    logger.error('[property] checkPropertyPaymentStatus error:', err.message);
    captureException(err);
...
module.exports = {
  ...
  createProperty,
  updateProperty,
  getPropertyReputationForLandlord,
