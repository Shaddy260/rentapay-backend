...
const { getExpiredPropertyIds } = require('../utils/subscriptionGate');
...
async function autoFillPropertyContact(manager, propertyIds) {
  ...
  if (manager.role_level === 'caretaker') {
    const { data: props } = await supabase
      ...
      .in('id', propertyIds);
    // PERFORMANCE FIX: parallelized (bounded concurrency) instead of
    // one sequential update per property - see the fix in
    // getExpiredPropertyIds for the same underlying pattern.
    await runInBatches(
      (props || []).filter((p) => !p.caretaker_name && !p.caretaker_phone),
      (p) => supabase.from('properties').update({ caretaker_name: manager.full_name, caretaker_phone: manager.phone }).eq('id', p.id),
      { concurrency: 10 }
    );
  } else {
    const { data: props } = await supabase
      ...
      .in('id', propertyIds);
    await runInBatches(
      (props || []).filter((p) => !p.primary_contact_manager_id),
      (p) => supabase.from('properties').update({ primary_contact_manager_id: manager.id }).eq('id', p.id),
      { concurrency: 10 }
    );
  }
}
...
async function addManager(req, res) {
  try {
    ...
    const { fullName, propertyIds, gender } = req.body;
    let { email } = req.body;
    let { phone, whatsappNumber } = req.body;
    let { roleLevel } = req.body;

    const required = { fullName, phone, email, whatsappNumber };
    ...
    // FIX: this uniqueness check never existed for email at all -
    // only phone was ever checked here, so two active managers/
    // caretakers (or a manager and an active tenant) could silently
    // share the same email, and reputation/notification lookups keyed
    // by email could not tell them apart. Mirrors addTenant's
    // findEmailConflict call, with the same is_active-scoped archive
    // exception (an archived manager/caretaker's email is free to
    // reuse under a new landlord).
    const emailConflict = await findEmailConflict(email, 'manager');
    ...
    if (ids.length) {
      ...
      // Assigning staff to a property is a property-level action -
      // an expired property can't gain a new manager/caretaker just
      // because some OTHER property on the account is still active.
      // PERFORMANCE FIX: batched into a fixed number of queries
      // instead of one round trip per property id (see
      // getExpiredPropertyIds's own comment for why that mattered).
      const expiredIds = await getExpiredPropertyIds(landlordId, ids);
      if (expiredIds.length > 0) {
        return res.status(403).json({
          error: 'One of the selected properties has an expired RentaPay subscription. Renew it before assigning staff there - your other properties are unaffected.',
    ...
    const { data: manager, error } = await supabase
      ...
      .insert({
        ...
        // DIRECT REQUEST FIX: "when they use that email for first login
        // they get an error... future logins say wrong password though
        // it's right." A property manager/caretaker is added directly
        // by a landlord who already knows and vouches for them - unlike
        // a tenant or landlord signing themselves up, there was never a
        // real reason to also require a self-service OTP step before
        // they can log in. is_verified used to default to false here
        // with no OTP ever generated for it to be checked against, so
        // login() always routed a brand-new manager/caretaker into the
        // "needsVerification" branch, sent them looking for a code that
        // didn't actually unblock anything useful, and left the account
        // stuck unverified indefinitely if that detour was ever
        // abandoned - every future login attempt (even with the exact
        // right password) kept hitting that same detour instead of
        // ever reaching the password check's real success path.
        // Verifying immediately, at creation, removes that entirely:
        // first login goes straight to "must change password" like the
        // temp-credentials email always described.
        is_verified: true,
    ...
    // No OTP in this email anymore - see is_verified: true above for
    // why there's nothing left to verify. Just the temp password and a
    // login link.
    const emailBody = templates.managerLoginCredentials(fullName, landlord?.full_name || 'Your landlord', tempPassword);
    try {
      await sendEmail(
        email,
      ...
      logger.error('[propertyManager] addManager: CRITICAL - login credentials email failed to send:', emailErr.message);
    ...
    await supabase.from('first_time_credentials').insert({
      ...
      otp: null, // no OTP step for managers/caretakers anymore - see is_verified: true above
    ...
    return res.status(201).json({
      ...
      tempCredentials: { phone, tempPassword },
    });
  } catch (err) {
    logger.error('[propertyManager] addManager error:', err.message);
...
async function listManagers(req, res) {
  try {
    ...
    logger.error('[propertyManager] listManagers error:', err.message);
    captureException(err);
...
async function updateManager(req, res) {
  try {
    ...
    if (email !== undefined) updateFields.email = email ? String(email).trim() : null;
    if (gender !== undefined) {
    ...
    logger.error('[propertyManager] updateManager error:', err.message);
    captureException(err);
...
async function updateAssignments(req, res) {
  try {
    ...
    if (propertyIds.length) {
      ...
      // PERFORMANCE FIX: batched (see the other call site above / the
      // helper's own comment) instead of one query per property id.
      const expiredIds = await getExpiredPropertyIds(landlordId, propertyIds);
      if (expiredIds.length > 0) {
        return res.status(403).json({
          error: 'One of the selected properties has an expired RentaPay subscription. Renew it before granting staff access there - your other properties are unaffected.',
          subscriptionExpired: true,
        });
      }
    }
    ...
    if (manager.email) {
      await sendEmail(manager.email, 'Your RentaPay property access was updated', wrapEmailHtml(templates.managerAssignmentsUpdated(manager.full_name))).catch((e) =>
        logger.warn('[propertyManager] updateAssignments: email failed (non-fatal):', e.message)
      );
    }
    ...
    logger.error('[propertyManager] updateAssignments error:', err.message);
    captureException(err);
  ...
  try {
    ...
    if (manager.email) {
      await sendEmail(manager.email, 'Your RentaPay access was removed', wrapEmailHtml(templates.managerRemoved(manager.full_name))).catch((e) =>
        logger.warn('[propertyManager] removeManager: email failed (non-fatal):', e.message)
      );
    }
    ...
    logger.error('[propertyManager] removeManager error:', err.message);
    captureException(err);
...
async function getMyAccess(req, res) {
  try {
    ...
    const { data: manager, error } = await supabase
      .from('property_managers')
      .select('id, full_name, phone, email, photo_url, landlord_id, role_level, gender, whatsapp_number')
      .eq('id', req.user.id)
      .single();
    ...
    logger.error('[propertyManager] getMyAccess error:', err.message);
    captureException(err);
