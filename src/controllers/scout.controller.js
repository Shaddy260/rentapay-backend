...
async function getVacancies(req, res) {
  try {
    const scoutId = req.user.id;
    // FIX (direct request: "scouts should not see occupied units...they
    // should only see vacant units"). Previously a scout could pass
    // ?status=occupied or ?status=all and see units that already have a
    // tenant - a scout's whole job is finding vacancies for prospective
    // tenants, so surfacing occupied units at all was a real leak of a
    // landlord's tenant/occupancy info to someone outside that
    // relationship. This is now hard-locked server-side (not just hidden
    // in the UI) so it can't be bypassed by calling the API directly with
    // a different status value.
    const statusFilter = 'vacant';
...
async function referUnit(req, res) {
  try {
    const scoutId = req.user.id;
    const { unitId, prospectName, prospectPhone } = req.body;
...
async function getMyReferrals(req, res) {
  try {
    ...
    const { data, error } = await supabase
      .from('scout_referrals')
      .select('id, status, shared_at, viewed_at, placed_at, payout_status, payout_amount, payout_paid_at, prospect_name, prospect_phone, stage_updated_at, units(unit_name)')
      .eq('scout_id', scoutId)
...
// ---------------------------------------------------------------------
// SCOUT: leaderboard - "gives scouts a reason to check back in" the
// same way self-reported stages do, but competitive instead of just
// personal. Ranked by all-time confirmed placements (the number that
// actually justifies the subscription fee, same weighting
// ScoutStatsPanel already gives it) with total shared as a tiebreaker.
// Milestone badges are simple placement thresholds - no separate
// table, computed on the fly here so there's nothing extra to keep in
// sync as placements roll in.
// ---------------------------------------------------------------------
const LEADERBOARD_MILESTONES = [
  { threshold: 25, label: 'Gold Scout' },
  { threshold: 10, label: 'Silver Scout' },
  { threshold: 5, label: 'Bronze Scout' },
  { threshold: 1, label: 'First Placement' },
];
function milestoneForPlacements(count) {
  return LEADERBOARD_MILESTONES.find((m) => count >= m.threshold)?.label || null;
}
...
async function getLeaderboard(req, res) {
  try {
    ...
    const { data, error } = await supabase
      .from('scout_referrals')
      .select('scout_id, status, scouts(full_name, photo_url)')
      .in('status', ['placed', 'shared', 'viewed_by_landlord', 'contacted', 'viewing_scheduled'])
      .order('shared_at', { ascending: false })
      .limit(5000);
    if (error) throw error;
    ...
    for (const row of data || []) {
      ...
      const entry = byScout.get(row.scout_id) || {
        scoutId: row.scout_id,
        name: row.scouts?.full_name || 'Scout',
        photoUrl: row.scouts?.photo_url || null,
        placements: 0,
        shared: 0,
      };
      entry.shared += 1;
      if (row.status === 'placed') entry.placements += 1;
      byScout.set(row.scout_id, entry);
    }

    const ranked = Array.from(byScout.values())
      .filter((s) => s.placements > 0 || s.shared > 0)
      .sort((a, b) => b.placements - a.placements || b.shared - a.shared)
      .map((s, i) => ({ ...s, rank: i + 1, milestone: milestoneForPlacements(s.placements) }));

    const top10 = ranked.slice(0, 10);
    const me = ranked.find((s) => s.scoutId === scoutId) || null;

    return res.json({
      leaderboard: top10,
      me: me ? { rank: me.rank, placements: me.placements, shared: me.shared, milestone: me.milestone } : null,
      milestones: LEADERBOARD_MILESTONES,
    });
  } catch (err) {
    console.error('[scout] getLeaderboard error:', err.message);
    return res.status(500).json({ error: 'Failed to load leaderboard.' });
  }
}

// ---------------------------------------------------------------------
// SCOUT: advance a referral's own lead-pipeline stage (shared ->
// contacted -> viewing_scheduled). Deliberately scout-self-reported
// and forward-only - the whole point ("gives scouts a reason to check
// back in") is letting a scout show their own follow-up work without
// waiting on the landlord to confirm anything. 'placed' is explicitly
// NOT settable here: it stays automatic-only (creditPlacementIfEligible
// in scoutReferral.service.js, triggered when the unit actually goes
// occupied) so a payout can never be claimed off a self-declared stage
// alone.
// ---------------------------------------------------------------------
const SELF_REPORTED_STAGE_ORDER = ['shared', 'contacted', 'viewing_scheduled'];

async function advanceReferralStage(req, res) {
  try {
    const scoutId = req.user.id;
    const { referralId } = req.params;
    const { stage } = req.body;

    if (!SELF_REPORTED_STAGE_ORDER.includes(stage) || stage === 'shared') {
      return res.status(400).json({ error: 'stage must be "contacted" or "viewing_scheduled".' });
    }
    ...
    if (referral.scout_id !== scoutId) return res.status(403).json({ error: 'You can only update your own referrals.' });

    if (referral.status === 'placed' || referral.status === 'expired') {
      return res.status(400).json({ error: 'This referral has already concluded and can no longer be updated.' });
    }

    // 'viewed_by_landlord' is an orthogonal landlord-side signal, not
    // a scout-progress stage - treat it the same as 'shared' for
    // ordering purposes so a landlord opening the referral doesn't
    // block the scout from logging their own follow-up.
    const currentIndex = SELF_REPORTED_STAGE_ORDER.indexOf(referral.status === 'viewed_by_landlord' ? 'shared' : referral.status);
    const targetIndex = SELF_REPORTED_STAGE_ORDER.indexOf(stage);
    if (currentIndex === -1 || targetIndex <= currentIndex) {
      return res.status(400).json({ error: 'Stage can only move forward - this referral is already past that point.' });
    }

    const { error } = await supabase
      .from('scout_referrals')
      .update({ status: stage, stage_updated_at: new Date().toISOString() })
      .eq('id', referralId);
    if (error) throw error;

    logActivity({ actorType: 'scout', actorId: scoutId, action: 'referral_stage_advanced', targetType: 'scout_referral', targetId: referralId, metadata: { stage } });

    return res.json({ message: 'Referral updated.', status: stage });
  } catch (err) {
    console.error('[scout] advanceReferralStage error:', err.message);
    return res.status(500).json({ error: 'Failed to update the referral.' });
  }
}
