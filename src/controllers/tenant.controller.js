...
async function addTenant(req, res) {
  try {
    ...
    // than being buried in account setup. Sent regardless of whether
    // this email has prior ratings elsewhere: a brand-new tenant gets
    // told their profile now exists and will follow them; a returning
    // one (by email, across landlords) gets shown what already
    // follows them into this new tenancy. This is the "tenant sees
    // their own reputation profile too" transparency principle from
    // the reputation notes - never silently withheld from them.
    try {
      ...
      console.error('[tenant] addTenant: tenancy reputation email failed to send:', repEmailErr.message);
    }
...
async function editOwnProfile(req, res) {
  try {
    ...
    console.error('[tenant] editOwnProfile error:', err.message);
...
async function rateTenant(req, res) {
  try {
    ...
    const allowedCategories = ['overall', 'payment', 'property_care', 'communication', 'conduct'];
    ...
    const { data: savedRating, error } = await supabase
      ...
      .upsert(
        ...
        { onConflict: 'landlord_id,tenant_email,category' }
      )
      .select()
      .single();
    if (error) throw error;
    ...
    console.error('[tenant] rateTenant error:', err.message);
    return res.status(500).json({ error: 'Failed to save rating.' });
  }
}

/** Portable reputation for a single tenant, for the tenant detail view. */
async function getTenantReputation(req, res) {
  try {
    ...
    console.error('[tenant] getTenantReputation error:', err.message);
...
async function listTenantReputations(req, res) {
  try {
    ...
    console.error('[tenant] listTenantReputations error:', err.message);
...
async function rateLandlord(req, res) {
  try {
    ...
    console.error('[tenant] rateLandlord error:', err.message);
    return res.status(500).json({ error: 'Failed to save rating.' });
  }
}
...
async function getMyLandlordReputation(req, res) {
  try {
    ...
    console.error('[tenant] getMyLandlordReputation error:', err.message);
