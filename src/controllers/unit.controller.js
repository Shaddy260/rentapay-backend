...
async function bulkCreateUnits(req, res) {
  try {
    const landlordId = effectiveLandlordId(req);
    const { units: unitsInput, propertyId } = req.body;
    ...
    if (toInsert.length > 0) {
      ...
      if (error) {
        console.error('[unit] bulkCreateUnits insert error:', error.message);
        return res.status(500).json({ error: 'Failed to save units. None were added - try again.' });
      }
      inserted = data || [];
    }

    // Activity log + scout vacancy notifications don't need to block
    // the response - the units are already saved, so let the person
    // move on to the next wizard step immediately.
    ...
    ).catch((err) => console.error('[unit] bulkCreateUnits scout notify error:', err.message));

    return res.status(201).json({ units: inserted, skipped });
  } catch (err) {
    console.error('[unit] bulkCreateUnits error:', err.message);
