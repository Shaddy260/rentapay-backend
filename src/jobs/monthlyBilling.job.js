...
async function runMonthlyBilling() {
  logger.info('[cron] Running monthly billing check...', new Date().toISOString());
  ...
  if (error) {
    logger.error('[cron] monthlyBilling: failed to fetch tenants:', error.message);
  ...
  await runInBatches(
    ...
    {
      concurrency: 10,
      onError: (err, tenant) => { logger.error(`[cron] monthlyBilling: failed to bill tenant ${tenant.id}:`, err.message); captureException(err); },
    }
  );

  logger.info('[cron] Monthly billing check complete.');
}
...
function startMonthlyBillingJob() {
  ...
  logger.info('[cron] Monthly billing job scheduled (daily at 00:01, plus once now on startup) - also applies any due scheduled rent changes. No interest accrual.');
}
