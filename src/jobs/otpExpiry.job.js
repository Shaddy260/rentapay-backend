...
async function sweepExpiredOtps() {
  ...
  if (firstTimeErr) { logger.error('[cron] otpExpiry: failed clearing expired first_time_credentials:', firstTimeErr.message); captureException(firstTimeErr); }
  ...
  if (resetLogErr) { logger.error('[cron] otpExpiry: failed clearing expired password_reset_requests:', resetLogErr.message); captureException(resetLogErr); }
  ...
  for (const table of ['landlords', 'tenants', 'property_managers']) {
    ...
    if (error) { logger.error(`[cron] otpExpiry: failed clearing expired otp columns on ${table}:`, error.message); captureException(error); }
  }
}
...
function startOtpExpiryJob() {
  ...
  logger.info('[cron] OTP/temp-password expiry sweep scheduled (every minute).');
}
