...
async function sweepOldPaymentConfirmations() {
  ...
  if (error) {
    logger.error('[cron] paymentConfirmationRetention: failed to sweep old records:', error.message);
    captureException(error);
    return;
  }

  logger.info(`[cron] paymentConfirmationRetention: deleted ${deletedRows?.length || 0} confirmed/rejected payment confirmation(s) older than 6 months.`);
}
...
function startPaymentConfirmationRetentionJob() {
  ...
  logger.info('[cron] Payment confirmation retention sweep scheduled (daily).');
}
