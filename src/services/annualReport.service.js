...
function drawMonthlyTable(doc, monthly, { includeNet = true, includeExpected = true } = {}) {
  const tableTop = doc.y;
  const colWidths = includeNet
    ? (includeExpected ? [80, 110, 110, 110, 110] : [90, 145, 145, 145])
    : (includeExpected ? [90, 110, 110, 110] : [90, 220, 220]);
  const headers = includeNet
    ? (includeExpected ? ['Month', 'Expected', 'Collected', 'Expenses', 'Net'] : ['Month', 'Collected', 'Expenses', 'Net'])
    : (includeExpected ? ['Month', 'Expected', 'Collected', 'Expenses'] : ['Month', 'Collected', 'Expenses']);
  let x = 50;
  ...
  monthly.forEach((m) => {
    ...
    const cells = includeNet
      ? (includeExpected ? [m.label, KES(m.expected), KES(m.collected), KES(m.expenses), KES(m.net)] : [m.label, KES(m.collected), KES(m.expenses), KES(m.net)])
      : (includeExpected ? [m.label, KES(m.expected), KES(m.collected), KES(m.expenses)] : [m.label, KES(m.collected), KES(m.expenses)]);
...
function generateAnnualPortfolioPdf(res, { landlordName, generatedAt, report }) {
  ...
  const cardY = doc.y;
  const cards = [
    { label: 'Expected', value: KES(report.portfolioTotals.expected) },
  ...
  report.properties.forEach((prop, idx) => {
    ...
    doc.fontSize(9).fillColor('#888').text(`Expected ${KES(prop.totalExpected)} · Collected ${KES(prop.totalCollected)} · Expenses ${KES(prop.totalExpenses)} · Net ${KES(prop.totalNet)}`);
    doc.moveDown(0.4);
