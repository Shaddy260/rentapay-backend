...
// completed activation - so every landlord and tenant payment
...
if (MOCK_DARAJA) {
  logger.warn('[daraja] MOCK_DARAJA=true - all STK pushes are FAKE. Do not use in production.');
}
 ...
 * @param {number} amount
 * @param {string} accountReference - e.g. unit code, used to identify what's being paid for
...
async function initiateSTKPush({ phoneNumber, amount, accountReference, transactionDesc }) {
  if (MOCK_DARAJA) {
    ...
    logger.info(`[daraja MOCK] Fake STK push -> phone=${phoneNumber} amount=${amount} ref=${accountReference} checkoutRequestId=${fakeId}`);
    return {
