...
async function sendEmail(to, subject, htmlBody) {
  if (!RESEND_API_KEY) {
    logger.warn('[email] RESEND_API_KEY not set - skipping email send. Subject would have been:', subject);
  ...
  try {
    ...
    logger.error('[email] Failed to send email (non-fatal - message was NOT delivered):', err.response?.data || err.message);
