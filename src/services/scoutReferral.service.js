...
//
// FIX (scout lead pipeline): this used to require status === 'shared'
// exactly, and always overwrote status to 'viewed_by_landlord' - fine
// when 'shared' was the only stage before 'placed', but now a scout
// may have already self-advanced the referral to 'contacted' or
// 'viewing_scheduled' by the time the landlord opens it. viewed_at is
// now set unconditionally (still idempotent - a second view just
// re-stamps the same field), and status only gets bumped to
// 'viewed_by_landlord' if the scout hasn't already moved it further
// along - a landlord opening the referral should never rewind a
// scout's own recorded progress.
// ---------------------------------------------------------------------
async function markReferralViewed(referralId) {
  ...
  if (!referral || referral.status === 'placed' || referral.status === 'expired') return; // nothing to do

  const update = { viewed_at: referral.viewed_at || new Date().toISOString() };
  if (referral.status === 'shared') update.status = 'viewed_by_landlord';
