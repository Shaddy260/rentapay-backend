...
function initSentry() {
  ...
  if (!dsn) {
    // FIX: this used to unconditionally say "this is fine in dev" even
    // when NODE_ENV=production - which is actively misleading on a
    // real deployment (exactly what showed up in the logs: a
    // "this is fine in dev" line sitting right next to a genuine
    // production warning about Daraja sandbox mode). It's still a
    // harmless no-op either way - errors just aren't reported anywhere
    // but the console - but the message now says so honestly instead
    // of implying "you don't need to worry about this" on a live server.
    if (process.env.NODE_ENV === 'production') {
      logger.warn('[sentry] SENTRY_DSN not set - error tracking disabled. Set SENTRY_DSN to get error reporting on this production server.');
    } else {
      logger.info('[sentry] SENTRY_DSN not set - error tracking disabled (this is fine in dev).');
    }
    return;
  }
  ...
  try {
    ...
    logger.warn('[sentry] Failed to initialize (continuing without error tracking):', err.message);
    sentryClient = null;
  }
}
...
function captureException(err) {
  ...
  try {
    ...
    logger.warn('[sentry] captureException failed (non-blocking):', captureErr.message);
  }
}
