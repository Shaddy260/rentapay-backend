...
//
// "Live push" - fires a real browser/OS push notification (via the
// Web Push protocol + VAPID) so the tenant/landlord/manager sees it
// even when the portal tab isn't open. Called from notify.service.js
// for every notification by default (see the `urgent` flag there) -
// this file itself has no concept of which event types "deserve" a
// push; that decision lives entirely in notify.service.js's callers.
//
...
if (VAPID_PUBLIC_KEY && VAPID_PRIVATE_KEY) {
  ...
  logger.warn('[webpush] VAPID_PUBLIC_KEY/VAPID_PRIVATE_KEY not set - live push notifications are disabled.');
}
...
async function getNotificationStyle(recipientType, recipientId) {
  ...
  if (!table) return 'ring'; // admin: no preference column, use the OS default
 ...
 *
 * @param {'landlord'|'manager'|'tenant'} recipientType
...
async function sendPushToRecipient(recipientType, recipientId, payload) {
  ...
  try {
      ...
      subs.map(async (sub) => {
        try {
          ...
          if (sendErr.statusCode === 404 || sendErr.statusCode === 410) {
            ...
            logger.error(`[webpush] send failed for ${recipientType} ${recipientId}:`, sendErr.message);
    ...
    logger.error(`[webpush] sendPushToRecipient error for ${recipientType} ${recipientId}:`, err.message);
