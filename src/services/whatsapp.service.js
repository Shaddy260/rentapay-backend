...
async function sendWhatsAppMessage(phoneNumber, message) {
  logger.info(`[whatsapp] (disabled) would have sent to ${phoneNumber}: ${message}`);
...
async function sendBulkWhatsAppMessage(phoneNumbers, message) {
  logger.info(`[whatsapp] (disabled) would have bulk-sent to ${phoneNumbers.length} numbers: ${message}`);
