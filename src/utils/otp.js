...
//     tenant/manager/caretaker is given to log in for the very
...
// Live, in-session email-ownership check (tenant self-onboarding
// public form verifying they actually control the email they typed) -
// longer than the password-reset window since filling in the rest of
// the form around it can reasonably take a few minutes, shorter than
// the 24h invite window since there's no reason for this one to
// linger.
function getEmailVerificationOTPExpiry() {
  const expiry = new Date();
  expiry.setMinutes(expiry.getMinutes() + EMAIL_VERIFY_OTP_EXPIRY_MINUTES);
  return expiry;
}
