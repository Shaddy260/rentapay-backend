...
/**
 * PERFORMANCE FIX (platform audit): batched counterpart to
 * isSubscriptionExpiredFor for the common "check expiry for several
 * properties at once" case (e.g. assigning a manager/caretaker to
 * multiple properties in one request). The naive approach - calling
 * isSubscriptionExpiredFor in a loop, one `await` per property id -
 * turns into N sequential round trips to Supabase for N properties;
 * for a landlord with a large portfolio that's N*~50-100ms of pure
 * network latency stacked serially before the request can even start
 * doing its actual work. This does it in exactly 2 queries total
 * (one batched fetch of all the properties, one landlord fetch for
 * whichever of them don't have their own per-property clock),
 * regardless of how many property ids are passed in.
 *
 * @param {string} landlordId
 * @param {string[]} propertyIds
 * @returns {Promise<string[]>} the subset of propertyIds that are expired
 */
async function getExpiredPropertyIds(landlordId, propertyIds) {
  ...
  const ownClockIds = ids.filter((id) => byId.get(id)?.unit_limit != null);
  const sharedClockIds = ids.filter((id) => !ownClockIds.includes(id));

  const expired = ownClockIds.filter((id) => byId.get(id)?.subscription_status === 'expired');

  if (sharedClockIds.length > 0) {
    const { data: landlord } = await supabase
      .from('landlords')
      .select('subscription_status')
      .eq('id', landlordId)
      .maybeSingle();
    if (landlord?.subscription_status === 'expired') expired.push(...sharedClockIds);
  }
