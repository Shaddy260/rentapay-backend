...
async function applyUnitLimitChange({ landlordId, propertyId, newLimit, actorType, actorId, excludePropertyIds }) {
  ...
  if (error) {
    logger.error('[unitLimitEnforcement] failed to load units:', error.message);
    return;
  }
...
async function reconcileLandlordUnitLimits(landlordId) {
  ...
  try {
    ...
    logger.error('[unitLimitEnforcement] reconcileLandlordUnitLimits failed (non-blocking):', err.message);
  }
}
